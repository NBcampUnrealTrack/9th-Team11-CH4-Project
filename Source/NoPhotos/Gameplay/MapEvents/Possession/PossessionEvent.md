# 빙의 이벤트: 등 뒤 유령과 WASD 이동 반전

이벤트 동안 플레이어마다 등 뒤 유령을 표시하고 **전후(W↔S)와 좌우(A↔D) 이동을 모두 반전**합니다. 마우스 시점, 점프, 사용 키는 유지합니다. 피해/공격/AI, 사진 판정, UI는 변경하지 않습니다.
...
Ghost Class가 비어 있거나 추상 클래스이면 `LogNPPossessionEvent` 경고가 한 번 나오며 유령은 표시하지 않습니다. 입력 반전과 이벤트 Duration은 외형 유무와 별개로 유지됩니다.
...
10. 유령 자체는 충돌을 만들지 않는지. Reverse Horizontal Input을 끈 이벤트는 유령만 표시하는지.
