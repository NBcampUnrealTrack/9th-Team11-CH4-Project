...
이벤트의 `GoblinClass`에 지정된 **고블린 캐릭터 BP → Class Defaults → Goblin | Photo | Reward**에서 설정합니다. 이벤트 Definition이 아니라 실제 소환되는 고블린 BP의 설정입니다.

- `Photo Relic Drop Count`: 촬영 한 번마다 드랍할 개수. 기본 **1**.
- `Defeat Relic Drop Count`: 사진 HP가 0이 될 때 추가 드랍할 개수. 기존 보상량을 유지하기 위해 기본 **0**.

마지막 촬영은 두 설정을 합산합니다. 예를 들어 촬영 2 / 처치 4이면 일반 촬영 때 2개, 마지막 촬영 때 6개입니다. 각 값을 0으로 두면 해당 보상만 끌 수 있습니다. 설정 범위는 각각 0~100개입니다.

각 유물은 기존 `RelicDropTable`에서 따로 무작위 선택하며, 테이블을 사용할 수 없으면 기존 `PhotographedRelicClass`를 사용합니다. 생성/물리 발사는 서버에서만 처리합니다. 이벤트 시간 만료나 일반 퇴장은 처치로 간주하지 않으며, HP 0 이후 추가 촬영으로 처치 보상을 다시 받을 수 없습니다. `GetSpawnedPhotoRelic`은 가장 최근에 생성된 유물 하나를 반환합니다.

자동화 테스트 `NoPhotos.MapEvents.Goblin.RelicDropCounts`에 촬영별 개수, 마지막 촬영 합산, 처치 중복 방지, 보상 끄기, 일반 퇴장 제외를 추가했습니다. 빌드/실행 검증은 하지 않았습니다.
