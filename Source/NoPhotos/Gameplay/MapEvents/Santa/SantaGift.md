...
3. 바닥에 닿으면 낙하를 멈추고 개봉 연출을 시작합니다. 분리 메시를 지정하면 닫힌 상자를 본체+뚜껑으로 교체하고 뚜껑을 들어 올리며 회전합니다.
4. 기본 1초 뒤, 서버가 DA의 유물 후보 중 **하나를 균등 추첨하여 한 번만 생성**합니다. 기존 `ANPBaseRelic::ReleaseWithVelocityImpulse`로 전시 상태를 해제하고 물리를 켭니다.
5. 열린 상자는 기본 3초 후 제거합니다. 유물은 선물/산타 이벤트와 무관하게 월드에 남습니다.

산타가 떠나거나 이벤트를 수동 종료하면 이후 투하만 취소합니다. 이미 투하한 선물은 계속 착지/개봉합니다. 선물과 유물은 Persistent Level에 생성하므로 서버 전용 경로 레벨의 언로드에도 제거되지 않습니다.
...
- `CollisionBox`는 Query Only / PhysicsBody이며 WorldStatic·WorldDynamic을 Block, Pawn·PhysicsBody 등은 Ignore입니다. 선물끼리, 플레이어, 기존 유물에 닿아서 개봉하지 않습니다.
...
| BP 설정 | 기본값 | 의미 |
| --- | --- | --- |
| Opening Duration | 1초 | 착지부터 개봉 완료 및 유물 생성까지 시간 |
...
- `Count=5`, 구간 `0.1~0.9`이면 진행률 **10%, 30%, 50%, 70%, 90%**에 투하합니다. Duration 30초라면 약 3, 9, 15, 21, 27초입니다.
...
- 개봉 상태/착지 위치/서버 시각을 함께 복제하므로 개봉 중 접속한 클라이언트도 해당 진행 상태를 표시합니다. 클라이언트는 랜덤 추첨이나 유물 생성을 수행하지 않습니다.
...
3. 착지 전에는 유물이 없고, 개봉 완료 후 상자당 서버 유물 하나만 생기는지.
4. Listen/Dedicated Server + 클라이언트 2개에서 낙하·개봉·유물 수가 일치하는지.
5. 낙하 중/개봉 중 접속해 위치와 개봉 상태가 맞는지.
6. 마지막 상자가 떨어지는 중 이벤트가 끝나도 계속 착지/개봉하고 유물이 남는지.
