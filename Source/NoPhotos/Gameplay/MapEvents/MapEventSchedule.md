...
개수뿐 아니라 **이벤트 순서 전체를 시작 시 서버에서 확정**합니다. 카탈로그에서 `Enabled`가 켜진 일반 이벤트를 동일 확률로 추첨하며, 초대형 이벤트는 제외합니다. A/B 타입은 자동 순서를 제한하지 않습니다. 동일 Event Id는 한 번만 선택하며, ID가 없으면 Definition 에셋, Definition도 없으면 액터 클래스로 중복을 구분합니다. 서로 다른 종류라면 고유한 Event Id를 설정하세요. 실행 시 재추첨하지 않습니다.
