...
- 영역 안의 로컬 플레이어 카메라에만 거리 기반 안개를 추가합니다. 진입/이탈 시 기본 1초 동안 강도가 증가/감소합니다. 제한 완료 시 전방 3m부터 짙어지고 10m부터 안개색으로 덮습니다. FOV나 맵 전체 안개는 변경하지 않습니다. 기존 시야 머티리얼에는 `FogStrength` 파라미터를 추가해야 합니다.
